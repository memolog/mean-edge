...
class HostActionAccessor {
  constructor(actionExpression, getter) {
    this.actionExpression = actionExpression;
    this.getter = getter;
  }
  subscribe(view, boundElementIndex, directive) {
    var eventEmitter = this.getter(directive);
    return ObservableWrapper.subscribe(eventEmitter, (actionObj) => view.callAction(boundElementIndex, this.actionExpression, actionObj));
  }
}
...
export class ElementInjector extends TreeNode {
  ...
  instantiateDirectives(lightDomAppInjector, host, preBuiltObjects) {
    ...
    if (isPresent(shadowDomAppInjector)) {
      ...
      if (isPresent(publishAs) && publishAs.length > 0) {
        this._shadowDomAppInjector = shadowDomAppInjector.resolveAndCreateChild(ListWrapper.map(publishAs, (token) => {
  ...
  dynamicallyCreateComponent(componentDirective, parentInjector) {
